...
def load_data(batch_size=32, valid_set_size=0.025, test_set_size=0.025, file_path='', data_version='', kept_libraries=None, canonical_pas=False, no_dse_canonical_pas=False, sampleSize=None) :

    #Load plasmid data
    filePath = os.path.join(file_path, 'apa_plasmid_data' + data_version)
    plasmid_dict = load(filePath)
            ...
            inputs = [
                {
                    ...
                    'source_type' : 'dataframe',
                    'source' : 'df',
                    'extractor' : SequenceExtractor('padded_seq', start_pos=180, end_pos=180 + 205),
                    'encoder' : OneHotEncoder(seq_length=205),
                    'dim' : (205, 4, 1),
                    'sparsify' : False
                },
            ...
            outputs = [
                {
                    ...
                    'source_type' : 'matrix',
                    'source' : 'cuts',
                    'extractor' : CountExtractor(start_pos=180, end_pos=180 + 205, static_poses=[-1], sparse_source=False),
                    'transformer' : lambda t: iso_normalizer(t),
                    'dim' : (1,),
                    'sparsify' : False
                },
